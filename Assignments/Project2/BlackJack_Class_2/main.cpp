 ...
 * Created on December 7, 2021, 8:00 PM
 * Purpose:  Second version of Class conversion
 *           -Converted Deck struct to Deck class
 *           -Call Deck class to create deck
 */
...
void crDeck(Deck &);    //Create deck of 52
string card(Player *, Deck &, int &);  //Assigns card to player
void calcTot(Player *);                 //Calculate total of hand
void hit(Player *, Deck &, int &);     //Ask user if want another card
void stats(Player *, Player *);  //Display win or loss
void chckAce(Player *);          //Check if players hand contains an ace
void shuffle(Deck &);           //Passes in deck and shuffles values in deck
...
void hit(Player *playr, Deck &deck, int &idx){
...
string card(Player *playr, Deck &deck, int &index){
    int elem=index;         //Element number from 0 to 12
    //Store the value of face card from deck into players hand
    playr->plyrHnd->cards[playr->plyrHnd->index]=deck.val(elem);
    //If card selected is an ace, record its index in hand array
    if(deck.face(elem)=="A")playr->plyrHnd->aceElem=playr->plyrHnd->index;
